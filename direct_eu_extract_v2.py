"""
DIRECT EU WASTE TRANSFERS EXTRACTION v2
========================================
Fixed version with explicit commit verification.
"""
import csv
import psycopg2
...
def extract_eu():
    ...
    conn.autocommit = True  # Use autocommit to prevent transaction block issues
    cur = conn.cursor()
    ...
    if doc:
        ...
        cur.execute("""
            ...
            VALUES ('eprtr', 'file://EU_WasteTransfers', %s, 'csv', 'direct_extract_v2', 'processing')
            RETURNING id
        """, (str(EU_FILE.resolve()),))
    ...
    with open(EU_FILE, 'r', encoding='utf-8-sig') as f:
        ...
        for row in reader:
            try:
                ...
                waste_class = row.get('wasteClassification', '').strip()
                treatment = row.get('wasteTreatment', '').strip()
                ...
                citation = f"EU E-PRTR WasteTransfers {year}: {facility[:50] if facility else 'Unknown'} - {material}"
                ...
                if count % batch_size == 0:
                    conn.commit()
                    if count % 10000 == 0:
                        print(f"   Processed {count:,} | Inserted {inserted:,}")
    ...
    # Verify
    cur.execute("SELECT COUNT(*) FROM waste_listings WHERE document_id = %s", (doc_id,))
